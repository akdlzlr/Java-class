...
public class BoardControllerTest extends BaseMvcTest<BoardController>{
	...
	public void testCreateSubmit() throws Exception{
		...
		request.param("writer", "sking")
				.param("password", "1234")
				.param("title", "단위 테스트 제목")
				.param("content","단위 테스트 내용");
		
		request.file(
				new MockMultipartFile("files", "test1.txt", "multipart/mixed", "업로드 테스트1".getBytes())
			);
		
		request.file(
				new MockMultipartFile("files", "test2.txt", "multipart/mixed", "업로드 테스트2".getBytes())
			);
