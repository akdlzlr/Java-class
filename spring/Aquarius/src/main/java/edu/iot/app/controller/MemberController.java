...
/*	@ResponseBody
	...
	public ResponseEntity<byte[]> getAvata(@PathVariable("userId") String userId) throws Exception {
		...
		try {
			...
			return new ResponseEntity<byte[]>(avata, headers, HttpStatus.OK); // 200 응답코드
		} catch (Exception e) {
			return new ResponseEntity<byte[]>(null, headers, HttpStatus.INTERNAL_SERVER_ERROR); // 500 응답코드
		}
	}
	...
	public String editSubmit(@Valid Member member, BindingResult result,
			@RequestParam("avata") MultipartFile mFile, HttpSession session) throws Exception{
